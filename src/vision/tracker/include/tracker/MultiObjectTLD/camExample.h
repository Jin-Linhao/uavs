...
#define DIRECTORY "/home/chencl/Desktop/NTU_Research_Group_Vision/videomaker/qt/maker-build-desktop-Qt_4_8_1_in_PATH__System__Release/"
...
bool learningEnabled = false, save = false, load = false, reset = false;
...
{
public:
    ssBox(int x,int y,int width, int height):x(x),y(y),width(width),height(height){}
...
MultiObjectTLD p(RESOLUTION_X, RESOLUTION_Y, settings);
#endif

void Init(int argc, char *argv[])
{  
    drawMode ^= DEBUG_DRAW_DETECTIONS;
    drawMode ^= DEBUG_DRAW_CROSSES;
    drawMode ^= DEBUG_DRAW_PATCHES;
    ...
    cvNamedWindow("MOT", 0); //CV_WINDOW_AUTOSIZE );

    CvSize wsize = {ivWidth, ivHeight};
    curImage = cvCreateImage(wsize, IPL_DEPTH_8U, 3);

    cvResizeWindow("MOT", ivWidth, ivHeight);
    cvSetMouseCallback("MOT", MouseHandler);

    #ifdef FORCE_RESIZING
    wsize = {RESOLUTION_X, RESOLUTION_Y};
    frame = cvCreateImage(wsize, IPL_DEPTH_8U, 3);
    #endif
...
string Images::findfile()
{
    ...
    return DIRECTORY+ss.str()+".png";
}

IplImage* Images::grab()
{
    IplImage* capframe;
    if(flag.any())
    {
        filename=findfile();
        capframe=cvLoadImage(filename.c_str());
        if (capframe==NULL)
            std::cout<<"read images in"<<filename<<" folds wrong!";
    }
    else
    {
...
#pragma omp parallel for
    ...
    vector<ssBox> box;
    for(int i=0;i<p.ivNObjects;i++)
        if((p.ivValid)[i]||(p.ivDefined)[i])
        {
            box.push_back(ssBox((p.ivCurrentBoxes)[i].x,(p.ivCurrentBoxes)[i].y,(p.ivCurrentBoxes)[i].width,(p.ivCurrentBoxes)[i].height));
            //cout<<i<<":"<<(p.ivCurrentBoxes)[i].x+(p.ivCurrentBoxes)[i].width/2<<" "
            //   <<(p.ivCurrentBoxes)[i].y+(p.ivCurrentBoxes)[i].height/2<<endl;
        }
  return box;
}
