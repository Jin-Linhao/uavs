...
void MultiObjectTLD::addObjects(std::vector<ObjectBox> obs)
{
  ...
  { 
    ...
    ivLKTracker.initFirstFrame(ivCurImage);//save image pyramid
    std::vector<Matrix> initNegPatches = ivFernFilter.addObjects(ivCurImage, obs);//compute search bounding boxes
    ...
    ivFernFilter.addObjects(ivCurImage, obs);//if not first frame, add objects directely
