        ...
        case rcmIfUsb:
            {
                ...
                //radioFd = open(destAddr, O_RDWR | O_NOCTTY | O_SYNC);
                radioFd = open(destAddr, O_RDWR | O_NOCTTY | O_NDELAY);
                //radioFd = open(destAddr, O_RDWR | O_NOCTTY);
...
int serWrite(const void *buf, int cnt)
{
    int i=write(radioFd, buf, cnt);
    if (i<0)
